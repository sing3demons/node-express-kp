...
export const HandlerSchema = (handler, hook) => ({
    handler,
    hook,
});
...
export default class AppServer extends AppRouter {
    ...
    validatorFactory(req, schema) {
        const errors = [];
        if (schema.body) {
            const C = TypeCompiler.Compile(schema.body);
        ...
        if (schema.params) {
            const C = TypeCompiler.Compile(schema.params);
        ...
        if (schema.query) {
            const C = TypeCompiler.Compile(schema.query);
        ...
        if (schema.headers) {
            const C = TypeCompiler.Compile(schema.headers);
